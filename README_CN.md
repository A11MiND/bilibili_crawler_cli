...
`status` 只读取本地 CSV 和 checkpoint，不发送网络请求：
...
公开发行只包含运行所需核心代码、包元数据、许可证和使用文档。运行输出、断点、Cookie、缓存、内部需求文档和测试资料不进入公开仓库。
...
CLI 在登录抓取前调用此方法。抓取期间，一级和二级翻页也使用独立的 60 秒认证新鲜度检查；到期后重新验证。这样可以区分“用户明确选择匿名”和“用户以为已登录但 Cookie 已失效”，并降低长任务中登录结果与匿名结果混合的风险。
