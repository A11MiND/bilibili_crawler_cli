...
class BilibiliClient:
    ...
    def fetch_root_page(
        ...
        if any(
            comment.parent is not None or comment.root != comment.rpid
            for comment in items
        ):
            raise ResponseFormatError(
                "root comment parent/root identity is invalid",
                _ROOT_REPLY_URL,
            )
    ...
    def fetch_child_page(
        ...
        if normalized_root != str(int(normalized_root)):
            raise ValueError("root_id must use canonical decimal digits")
        ...
        for attempt in range(1, self.max_attempts + 1):
            ...
            try:
                ...
                if (
                    ...
                    and parsed_retry_after > _MAX_RETRY_AFTER_SECONDS
                ):
                    raise TemporaryNetworkError(
                        "server requested a Retry-After interval longer "
                        f"than {_MAX_RETRY_AFTER_SECONDS:g} seconds",
                        endpoint=endpoint,
                        status=exc.code,
                        attempts=attempt,
                    ) from exc
