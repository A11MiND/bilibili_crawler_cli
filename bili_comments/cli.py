...
from .storage import Checkpoint, CheckpointStore, CsvStore, StorageError
...
def run_status(
    ...
    *,
    ...
    try:
        ...
        csv_data = _read_strict_csv_status(csv_path, checkpoint)
    except (StorageError, OSError, UnicodeError) as exc:
        base_data["consistent"] = False
        return _fail(
            "status",
            EXIT_LOCAL,
...
def _read_cookie_file(
    ...
    *,
    ...
    try:
        ...
        if nofollow is None:
            raise CookieConfigurationError(
                f"当前平台无法安全打开 Cookie 文件（缺少 O_NOFOLLOW）：{path}"
            )
        descriptor = os.open(
            expanded,
            os.O_RDONLY
        ...
        _validate_cookie_file_security(
            expanded,
            descriptor,
...
def _validate_cookie_file_security(
    path: Path,
    descriptor: int,
    *,
    ...
    if os.name != "posix" or not enforce_permissions:
        return
    if metadata.st_uid != os.getuid():
        raise CookieConfigurationError(f"Cookie 文件所有者不是当前用户：{path}")
    if metadata.st_nlink != 1:
        raise CookieConfigurationError(
            f"Cookie 文件必须只有一个硬链接：{path}"
        )
...
def _read_strict_csv_status(
    path: Path,
    checkpoint: Checkpoint,
) -> dict[str, int]:
    """Validate the complete CSV without applying resume-time truncation."""

    store = CsvStore(path, create=False)
    row_count = store.rows_written
    root_count = len(store.root_sequences)
    child_count = row_count - root_count
    ip_count = store.ip_location_count

    if store.committed_bytes != checkpoint.committed_bytes:
        raise StorageError(
            "checkpoint committed_bytes 与完整 CSV 文件大小不一致"
        )
    if row_count != checkpoint.rows_written:
        raise StorageError(
            "checkpoint rows_written 与 CSV 唯一评论数不一致"
        )
    if checkpoint.next_root_sequence != store.max_root_sequence + 1:
        raise StorageError(
            "checkpoint next_root_sequence 与 CSV 一级评论序号不一致"
        )

    unknown_completed_roots = set(
        checkpoint.completed_root_ids_in_page
    ) - set(store.root_sequences)
    if unknown_completed_roots:
        raise StorageError(
            "checkpoint 包含 CSV 中不存在的已完成一级评论"
        )
    if (
        checkpoint.current_root_id is not None
        and checkpoint.current_root_id not in store.root_sequences
    ):
        raise StorageError(
            "checkpoint 当前一级评论在 CSV 中不存在"
        )
    ...
    return {
        ...
        "file_bytes": store.committed_bytes,
        "uncommitted_bytes": 0,
    }
...
def _existing_progress_counts(bvid: str) -> tuple[int, int, int]:
    ...
    try:
        ...
        if checkpoint is None:
            return (0, 0, 0)
        counts = _read_strict_csv_status(csv_path, checkpoint)
    except (StorageError, OSError, UnicodeError):
        return (0, 0, 0)
    return (
        counts["row_count"],
        counts["root_count"],
