...
class CsvStore:
    ...
    def __init__(
        ...
        if committed_bytes is None:
            self._load_indexes()
            self.committed_bytes = self.path.stat().st_size
    ...
    def is_header_only(self) -> bool:
        ...
        try:
            return (
                not self.seen_ids
                and self.path.stat().st_size == len(_CSV_HEADER_BYTES)
                and self.path.read_bytes() == _CSV_HEADER_BYTES
            )
        except OSError as exc:
    ...
    def _load_indexes(self) -> None:
        try:
            with self.path.open("rb") as raw:
                self._validate_binary_csv_envelope(raw.read())
